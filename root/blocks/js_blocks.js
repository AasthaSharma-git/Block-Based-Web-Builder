...
Blockly.Blocks['js_variable_declare'] = {
    init: function() {
      this.appendDummyInput()
        .appendField("declare variable"); // Label for the block
      this.appendValueInput("VALUE")
        .setCheck(null) // Allow any type for value
  ...
  Blockly.JavaScript['js_variable_declare'] = function(block) {
    ...
    var value = Blockly.JavaScript.valueToCode(block, 'VALUE', Blockly.JavaScript.ORDER_ATOMIC) || '0'; // Get the value input
  ...
  Blockly.Blocks['js_event_listener'] = {
    init: function() {
      this.appendDummyInput()
          .appendField("Add event listener on")
  ...
  Blockly.Blocks['js_event_listenerDOMContentLoaded'] = {
    init: function() {
      this.appendDummyInput()
          .appendField("Add DOMContentLoaded event listener");
      this.appendStatementInput("DO") // Allow nested blocks for the action
          .setCheck(null); // Accept any type of blocks
      this.setColour(160); // Set block color
...
Blockly.Blocks['js_console_log'] = {
  init: function() {
    ...
    this.appendValueInput("TEXT")
      .setCheck(null) // Allow any type for the log message
      .appendField(":"); // Label to clarify input
    this.setPreviousStatement(true); // Allow connecting to previous blocks
    this.setNextStatement(true); // Allow connecting to next blocks
...
Blockly.JavaScript['js_console_log'] = function(block) {
  var text = Blockly.JavaScript.valueToCode(block, 'TEXT', Blockly.JavaScript.ORDER_ATOMIC) || '""'; // Get the log message input
...
Blockly.Blocks['js_function'] = {
  init: function() {
    this.appendDummyInput()
      .appendField("define function") // Label for the block
      .appendField(new Blockly.FieldTextInput("myFunction"), "NAME"); // Input for function name
    this.appendStatementInput("DO") // Allow nested blocks for the function body
      .appendField("do"); // Label for the function body
    this.setColour(230); // Set block color
...
Blockly.JavaScript['js_function'] = function(block) {
  ...
  var statements_do = Blockly.JavaScript.statementToCode(block, 'DO'); // Get the statements inside the function
...
Blockly.Blocks['js_if_else'] = {
  init: function() {
    ...
    this.appendStatementInput("DO") // Statement input for the if block
      .appendField("do"); // Label for the action if condition is true
    this.appendDummyInput()
      .appendField("else"); // Label for the else statement
    this.appendStatementInput("ELSE") // Statement input for the else block
      .appendField("do"); // Label for the action if condition is false
...
Blockly.Blocks['js_for_loop'] = {
  init: function() {
    ...
    this.setColour(120); // Set block color
